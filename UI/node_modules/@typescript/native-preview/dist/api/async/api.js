...
class TypeObject {
    ...
    constructor(data, client, snapshotId, objectRegistry) {
        ...
        if (data.aliasTypeArguments !== undefined)
    ...
    async getAliasSymbol() {
        ...
        if (cached)
            return cached;
        const data = await this.client.apiRequest("getAliasSymbolOfType", { snapshot: this.snapshotId, type: this.id });
        return data ? this.objectRegistry.getOrCreateSymbol(data) : undefined;
    }
    ...
    async getFreshType() {
        ...
        if (cached)
            return cached;
        const data = await this.client.apiRequest("getFreshTypeOfType", { snapshot: this.snapshotId, type: this.id });
        return data ? this.objectRegistry.getOrCreateType(data) : undefined;
    }
    async getRegularType() {
        ...
        if (cached)
            return cached;
        const data = await this.client.apiRequest("getRegularTypeOfType", { snapshot: this.snapshotId, type: this.id });
        return data ? this.objectRegistry.getOrCreateType(data) : undefined;
    }
    async getTypes() {
